...
class PortalButtonPawn {
    ...
    pressedButton() {
        ...
        let portalURL = this.resolvePath(this.actor._cardData.openPortalPath);
        this.say("openPortal", portalURL);
    }

    resolvePath(openPortalPath = "") {
        let { hostname } = window.location;
        if (hostname.endsWith("github.io")) {
            return `https://zfox23.github.io/croquet-sf/${openPortalPath}`;
        } else {
            return `/${openPortalPath}`;
        }
    }
}
